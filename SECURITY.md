...
(Apache behind `cloudflared` — see `DEPLOY.md`), public internet-facing.
...
| Manual "responded" tick (`CampaignShow::toggleResponded()`/`bulkMarkResponded()`) — privilege-gated same as the rest of the component, both scoped through `$this->campaign->recipients()->...` (the relationship, not a bare `CampaignRecipient::find`/`whereIn`), so a crafted recipient id from a different campaign can't be touched by a user who only has access to this one; XLSX/PDF export (`CampaignShow::export()`) is a component action reachable by any authenticated user viewing the page, same as the page itself (reads, not writes), scoped to `$this->campaign->recipients()` so it can't pull another campaign's rows, `abort_unless` on the format param, and the PDF Blade view renders every field through `{{ }}` | ✓ PASS |
