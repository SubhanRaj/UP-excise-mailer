...
This is an **on-premise, no-cloud** Laravel app living at `/home/subhan/Sites/UP-excise-mailer`
...
2. **Permissions** — `chmod o+x /home/subhan` if not already done (it will be, from the sibling
...
3. **`ProtectHome` override** — Ubuntu's `apache2.service` ships `ProtectHome=read-only`, which
   ...
   (shared across all three apps under `/home/subhan`) — just add this app's paths to its
